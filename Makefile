...
docker-build-backend:
	cd backend && docker build --build-arg ALLOW_ORIGINS="$(ALLOW_ORIGINS)" -t unshorten-it-backend .
...
docker-build-push-backend: docker-setup-buildx
	cd backend && docker buildx build --platform linux/amd64,linux/arm64 --build-arg ALLOW_ORIGINS="$(ALLOW_ORIGINS)" -t $(BACKEND_IMAGE) --push .
